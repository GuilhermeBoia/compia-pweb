...
export function useCheckoutIntegration() {
  ...
  const completeOrderWithIntegration = useCallback(async () => {
    ...
    try {
      ...
      if (paymentResult.success) {
        ...
        try {
          addToPurchaseHistory(paidOrder);
        ...
        try {
          addToOrderManagement(paidOrder);
  ...
  }, [customer, payment, shipping, items, addToPurchaseHistory, addToOrderManagement, clearCart, setOrder, setCurrentStep]);
